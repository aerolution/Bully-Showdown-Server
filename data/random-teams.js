...
class RandomTeams extends Dex.ModdedDex {
	...
	checkBattleForme(template) {
		...
		if (!this.format.noMegas && (firstForme.isMega || firstForme.isPrimal)) return firstForme;
		return null;
	}
	...
	randomTeam() {
		...
		while (pokemonPool.length && pokemon.length < 6) {
			...
			if (teamDetails['zMove'] && item.zMove && this.format.id !== 'gen7randomultimatez') continue;
