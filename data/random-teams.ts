...
export class RandomTeams {
	...
	queryMoves(moves: string[] | null, hasType: {[k: string]: boolean} = {}, hasAbility: {[k: string]: boolean} = {}, movePool: string[] = []) {
		...
		const SpeedSetup = [
			'agility', 'autotomize', 'flamecharge', 'rockpolish', 'shiftgear', 'scaleshot', 
		];
		...
		const NoStab = [
			...
			'clearsmog', 'eruption', 'icywind', 'incinerate', 'meteorbeam', 'snarl', 'vacuumwave', 'voltswitch', 'waterspout', 'mindblown',
		];
	...
	randomSet(species: string | Species, teamDetails: RandomTeamsTypes.TeamDetails = {}, isLead = false, isDoubles = false): RandomTeamsTypes.RandomSet {
		...
		do {
			...
			for (const [k, moveId] of moves.entries()) {
				...
				case 'meteorbeam':
					...
					break;
				case 'mudshot':
					if (!hasMove['batonpass'] && !hasMove['nastyplot']) rejected = true;
					break;
				case 'rest':
				...
				case 'closecombat':
					...
					if (hasMove['thunderouskick']) rejected = true;
					break;
				...
				case 'fakeout':
					...
					break;
				case 'flashcannon': case 'pollenpuff':
					if (hasMove['substitute'] || hasMove['toxic'] && movePool.includes('substitute')) rejected = true;
					break;
				...
				case 'highjumpkick': case 'machpunch':
					...
					if (moveid === 'highjumpkick' && hasMove['closecombat']) rejected = true;
					break;
				...
				case 'protect':
					...
					if (counter.Status < 2 && !hasAbility['Hunger Switch'] && !hasAbility['Guts'] && !hasAbility['Quick Feet'] && !hasAbility['Speed Boost'] && !hasAbility['Moody'] && !hasAbility['Poison Heal'] && !isDoubles) rejected = true;
				...
				case 'thunderwave': case 'voltswitch':
					...
					if (hasMove['nuzzle'] || hasMove['discharge']) rejected = true;
					break;
				case 'toxic':
				...
				case 'fireblast': case 'magmastorm':
					...
					break;
				case 'overheat':
					if (hasMove['fireblast'] || hasMove['flareblitz'] || hasMove['lavaplume'] || isDoubles && hasMove['calmmind']) rejected = true;
					break;
				...
				case 'drainpunch':
					...
					if (hasMove['closecombat'] && !hasMove['bulkup'] || !hasType['Fighting'] && movePool.includes('swordsdance')) rejected = true;
					break;
				...
				case 'hurricane':
					...
					break;
				case 'poltergeist':
					if (hasMove['knockoff'] || hasMove['shadowbone']) rejected = true;
					break;
				case 'shadowball':
				...
				case 'shadowpunch':
					...
					break;
				case 'dragonpulse': case 'outrage': case 'spacialrend':
					if (hasMove['dracometeor'] && counter.Special < 4) rejected = true;
					if (hasMove['scaleshot'] && hasMove['swordsdance']) rejected = true;
					break;
				case 'dragonclaw':
				...
				if (!rejected && !move.damage && !isSetup && !move.weather && !move.stallingMove &&
					(isDoubles || (!['facade', 'lightscreen', 'reflect', 'sleeptalk', 'spore', 'substitute', 'switcheroo', 'toxic', 'whirlpool', 'teleport'].includes(moveid) && (move.category !== 'Status' || !move.flags.heal))) &&
		...
		if (abilities[1]) {
			...
			do {
				...
				if (this.format.randomBanlist && this.format.randomBanlist.includes(ability)) {
					rejectAbility = true;
				} else if (['Anger Point', 'Moody', 'Flare Boost', 'Hydration', 'Ice Body', 'Inner Focus', 'Insomnia', 'Liquid Voice', 'Quick Feet', 'Rain Dish', 'Snow Cloak', 'Steadfast', 'Cloud Nine'].includes(ability)) {
					rejectAbility = true;
					...
					rejectAbility = (hasMove['earthquake'] && hasMove['milkdrink']) || (hasAbility['Iron Fist'] && hasMove['drainpunch']);
		...
		} else if (ability === 'Toxic Boost' || ability === 'Guts' && (counter.Physical > 2 || isDoubles)) {
		...
		// Better than Leftovers
			...
			counter.damagingMoves.length >= 3 && !counter['damage'] && ability !== 'Sturdy' && !hasMove['clearsmog'] && !hasMove['foulplay'] && !hasMove['rapidspin'] && !hasMove['substitute'] && !hasMove['uturn'] && 
			!isDoubles && (!!counter['speedsetup'] || hasMove['trickroom'] || !!counter['drain'] || hasMove['psystrike'] || (species.baseStats.spe > 40 && species.baseStats.hp + species.baseStats.def + species.baseStats.spd < 275))
		) {
			item = 'Life Orb';
		} else if (counter.damagingMoves.length >= 4 && !counter['Dragon'] && !counter['Normal'] && !isDoubles) {
			item = 'Expert Belt';
	...
	randomTeam() {
		...
		while (pokemonPool.length && pokemon.length < 6) {
			...
			case 'Darmanitan':
				...
				break;
			case 'Magearna': case 'Toxtricity': case 'Zacian': case 'Zamazenta': case 'Urshifu':
